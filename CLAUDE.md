...
Key: `treecarbonxray_v1`. Schema: `{ projectTrees, horizon, selectedRegion }`. Saved debounced 500ms on any change. Restored on mount. Clear Project button removes it.
...
| `services/carbonCalculator.ts` | Growth math engine — `forecastTreeGrowth()` |
| `services/dataService.ts` | CSV parsers for TS1/TS2/TS3/TS6/TS9 |
| `services/speciesCatalog.ts` | Builds species catalog from TS9+TS6, loads YAML image map |
